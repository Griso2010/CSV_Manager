...
class MyCSV:
    ...
    def readToDict(self, file_path:None|str|os.PathLike = None, sep:str = ",", key_field_num:None|int = None ) -> dict:
        self.__reader(file_path, sep) # Вызывали наш метод открытия файла
        ...
        for next_row in self.file_rows: # Бежим по строчкам файла
            ...
            res.update({splitted[self.key_field_num]:single_object_dict}) # Создаеи и обновляем словарь
    ...
    def __select_key(self, key_field_num):
        ...
        else: self.key_field_num = key_field_num
    ...
    def __creating_file(self, file_path:None|str|os.PathLike = None):
        ...
        # Проверка на создание нового файла
            match input('Create new file?[y]: ').upper():
                ...
                case _: self.file_to_write = self.__select_file(file_path)
    ...
    def writeFromDict(self, data_to_write:dict, file_path:None|str|os.PathLike = None, sep:str = ","):
        # Вызываю метод создания файла, если путь у файлу не указан
        ...
        with open(self.file_to_write, 'w', encoding='utf-8') as f: f.write(string_to_write)
